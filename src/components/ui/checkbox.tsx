...
export function CheckboxIcon({ checked = false }: IconProps) {
  const color = checked ? '#AEAEAE' : '#E8313B';
  return (
    <MotiView
      style={{
        height: SIZE,
  ...
  return (
    ...
    >
      <MotiView
        ...
        transition={{
          opacity: { duration: 80, type: 'timing' },
          backgroundColor: { duration: 120, type: 'timing' },
        }}
      />
    </MotiView>
  );
...
export function SwitchIcon({ checked = false }: IconProps) {
  const trackColor = checked ? '#F7D9D9' : '#E7E7E7';
  const thumbColor = checked ? '#E8313B' : '#AEAEAE';
